...
export default function RequestsPage() {
  const { user } = useUser();
  const { requests, loading } = useRequests(user);
  ...
  return (
    <div className="pt-32 px-8 min-h-screen bg-[#FAFAFA] pb-16">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Category Filters */}
        <div className="mb-6 flex flex-wrap justify-center gap-2" role="group" aria-label="Filter requests by status">
        ...
        {/* Detailed View Modal */}
        {selectedRequest && (
          <RequestModal request={selectedRequest} user={user} onClose={handleCloseModal} />
        )}
      </div>
    </div>
  );
